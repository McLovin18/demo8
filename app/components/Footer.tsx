...
const socialLinks = [
  { href: "https://www.instagram.com/ppcellec/", label: "Instagram", Icon: IconInstagram },
];

// 👉 Información del negocio
const WHATSAPP_NUMBER = "593959030557"; // solo números, con código de país, sin '+' ni espacios
const WHATSAPP_DISPLAY = "+593 95 903 0557"; // como se muestra al usuario
      ...
      <footer className=" border-t border-white/10 text-white">
          ...
          <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
            ...
            <div className="flex flex-col items-center md:items-start text-center md:text-left gap-1">
              <span className="text-base font-bold tracking-wide text-white">
                PP<span className="text-[#F5821F]">CELL</span>
              </span>
              ...
              <div className="text-xs text-white/60 mt-1 max-w-[220px]">
                <p>Tecnología Celular</p>
                <p>Los mejores equipos • Envíos a todo el país</p>
                ...
                <ul className={styles.ftSocials}>
                  {socialLinks.map(({ href, label, Icon }) => (
                    <li key={label}>
                      <a
                        href={href}
                        className="flex items-center justify-center w-9 h-9 rounded-full border border-white/15 text-white transition-colors hover:bg-[#F5821F] hover:border-[#F5821F]"
                        target="_blank"
                        rel="noreferrer"
                        title={label}
            ...
            <div className="flex flex-col items-center md:items-end gap-2.5">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-2 text-xl text-white/70 hover:text-[#F5821F] transition-colors"
                onClick={() => trackLinkClick().catch(console.error)}
              >
        ...
        <div className={styles.ftCopyRow}>
          <p className="text-xs text-white/50">
            © {new Date().getFullYear()} PPCELL. Todos los derechos reservados.
          </p>
          <div className={styles.ftCopyRight}>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-xs text-white/50 hover:text-[#F5821F] transition-colors"
              onClick={() => trackLinkClick().catch(console.error)}
            >
